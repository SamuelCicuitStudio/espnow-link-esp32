...
constexpr uint32_t kSdSpiHz = 10000000UL;  // SEMU (stability-first for sustained OTA/log reads)
constexpr uint32_t kLogRotateSpiffsBytes = 1572864U;   // 1.5 MiB
constexpr uint32_t kLogRotateSdBytes = 52428800U;      // 50 MiB
constexpr const char* kSlaveSwVersion = PCAT_SEMU_SW_VER;
constexpr bool kVerbosePullRequestTrace = false;
const std::string kSlaveBuildId = app_owned::buildid::defaultBuildId(BUILD_ID, PCAT_SEMU_BUILD_TAG, kSlaveSwVersion);
...
std::unique_ptr<app_owned::SemuAppDescriptorProvider> g_role_provider;
app_owned::SlaveSchemaPackage g_role_schema{};
SlaveOtaBootstrap g_ota_bootstrap;
SlaveNodeBootstrap g_slave_bootstrap;

struct SemuRuntimeState {
  uint32_t step = 0U;
};

SemuRuntimeState g_semu_runtime{};

bool readSemuTelemetry(void* user, app_owned::SemuRuntimeTelemetrySnapshot& out) {
  auto* state = static_cast<SemuRuntimeState*>(user);
#if defined(ARDUINO)
  const uint32_t now = millis();
#else
  const uint32_t now = 0U;
#endif
  uint32_t step = (state != nullptr) ? state->step : 0U;
  step += 1U + ((now >> 6) & 0x07U);
  if (state != nullptr) {
    state->step = step;
  }

  const float phase = static_cast<float>(now % 18000U) / 18000.0f;
  out.env_temp_c = 21.0f + (8.0f * phase);
  out.env_hum_pct = 38.0f + (30.0f * phase);
  out.env_press_pa = 100850.0f + (260.0f * phase);
  out.lux = 120.0f + (680.0f * phase);

  constexpr uint8_t kPairCount = 4U;
  out.pair_count = kPairCount;
  out.valid_pair_mask = static_cast<uint8_t>((1U << kPairCount) - 1U);
...
void setup() {
  ...
  app_owned::SemuDescriptorAppConfig app_cfg{};
  app_cfg.device_type = PCAT_SEMU_DEV_TYPE;
  app_cfg.hw_version = PCAT_SEMU_HW_VER;
  app_cfg.sw_version = kSlaveSwVersion;
  app_cfg.build_id = kSlaveBuildId;
  app_cfg.default_device_name = PCAT_SEMU_DEF_NAME;
  app_cfg.runtime_user = &g_semu_runtime;
  app_cfg.read_telemetry = &readSemuTelemetry;
  g_role_provider.reset(new app_owned::SemuAppDescriptorProvider(
  ...
  g_role_schema = app_owned::makeSemuSlaveSchemaPackage(*g_role_provider, kCodecIdCompactIndexed);
  if (!g_role_schema.valid() || !g_role_schema.registerProfile()) {
    Serial.println("[SLAVE][PROFILE] app-owned SEMU schema registration failed");
    return;
  }

  SlaveNodeBootstrap::Config boot_cfg{};
  boot_cfg.manager_config = SlaveNodeBootstrap::defaultManagerConfig(1, PCAT_SEMU_DEV_TYPE, true, true, 5000);
