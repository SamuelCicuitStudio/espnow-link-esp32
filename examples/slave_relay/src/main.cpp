...
constexpr uint32_t kSdSpiHz = 10000000UL;  // RELAY (stability-first for sustained OTA/log reads)
constexpr uint32_t kLogRotateSpiffsBytes = 1572864U;   // 1.5 MiB
constexpr uint32_t kLogRotateSdBytes = 52428800U;      // 50 MiB
constexpr const char* kSlaveSwVersion = PCAT_RELAY_SW_VER;
constexpr bool kVerbosePullRequestTrace = false;
const std::string kSlaveBuildId = app_owned::buildid::defaultBuildId(BUILD_ID, PCAT_RELAY_BUILD_TAG, kSlaveSwVersion);
...
std::unique_ptr<app_owned::RelayAppDescriptorProvider> g_role_provider;
...
RelayRuntimeState g_relay_runtime{};

bool readRelayTelemetry(void* user, app_owned::RelayRuntimeTelemetrySnapshot& out) {
  auto* state = static_cast<RelayRuntimeState*>(user);
#if defined(ARDUINO)
  const uint32_t now = millis();
#else
  const uint32_t now = 0U;
#endif
  uint32_t seed = (state != nullptr) ? state->seed : 0U;
  seed = (seed + 1U + (now & 0x0FU)) & 0x0FU;
  if (state != nullptr) {
    state->seed = seed;
  }

  const float phase = static_cast<float>(now % 12000U) / 12000.0f;
  out.relay_bitmap = (1UL << (seed & 0x03U));
  if ((seed & 0x04U) != 0U) {
    out.relay_bitmap |= 0x10U;
  }
  out.env_temp_c = 22.5f + (6.0f * phase);
  out.valid = true;
  return true;
}
...
void setup() {
  ...
  app_owned::RelayDescriptorAppConfig app_cfg{};
  app_cfg.device_type = PCAT_RELAY_DEV_TYPE;
  app_cfg.hw_version = PCAT_RELAY_HW_VER;
  app_cfg.sw_version = kSlaveSwVersion;
  app_cfg.build_id = kSlaveBuildId;
  app_cfg.default_device_name = PCAT_RELAY_DEF_NAME;
  app_cfg.runtime_user = &g_relay_runtime;
  app_cfg.read_telemetry = &readRelayTelemetry;
  g_role_provider.reset(new app_owned::RelayAppDescriptorProvider(
  ...
  g_role_schema = app_owned::makeRelaySlaveSchemaPackage(*g_role_provider, kCodecIdCompactIndexed);
  if (!g_role_schema.valid() || !g_role_schema.registerProfile()) {
    Serial.println("[SLAVE][PROFILE] app-owned RELAY schema registration failed");
    return;
  }

  SlaveNodeBootstrap::Config boot_cfg{};
  boot_cfg.manager_config = SlaveNodeBootstrap::defaultManagerConfig(1, PCAT_RELAY_DEV_TYPE, true, true, 5000);
