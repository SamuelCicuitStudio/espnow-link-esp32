...
constexpr uint32_t kSdSpiHz = 10000000UL;  // SENS (stability-first for sustained OTA/log reads)
constexpr uint32_t kLogRotateSpiffsBytes = 1572864U;   // 1.5 MiB
constexpr uint32_t kLogRotateSdBytes = 52428800U;      // 50 MiB
constexpr const char* kSlaveSwVersion = PCAT_SENS_SW_VER;
constexpr bool kVerbosePullRequestTrace = false;
const std::string kSlaveBuildId = app_owned::buildid::defaultBuildId(BUILD_ID, PCAT_SENS_BUILD_TAG, kSlaveSwVersion);
...
std::unique_ptr<app_owned::SensAppDescriptorProvider> g_role_provider;
app_owned::SlaveSchemaPackage g_role_schema{};
SlaveOtaBootstrap g_ota_bootstrap;
SlaveNodeBootstrap g_slave_bootstrap;

struct SensRuntimeState {
  uint32_t seed = 0U;
};

SensRuntimeState g_sens_runtime{};

bool readSensTelemetry(void* user, app_owned::SensRuntimeTelemetrySnapshot& out) {
  auto* state = static_cast<SensRuntimeState*>(user);
#if defined(ARDUINO)
...
void setup() {
  ...
  app_owned::SensDescriptorAppConfig app_cfg{};
  app_cfg.device_type = PCAT_SENS_DEV_TYPE;
  app_cfg.hw_version = PCAT_SENS_HW_VER;
  app_cfg.sw_version = kSlaveSwVersion;
  app_cfg.build_id = kSlaveBuildId;
  app_cfg.default_device_name = PCAT_SENS_DEF_NAME;
  app_cfg.runtime_user = &g_sens_runtime;
  app_cfg.read_telemetry = &readSensTelemetry;
  g_role_provider.reset(new app_owned::SensAppDescriptorProvider(
  ...
  g_role_schema = app_owned::makeSensSlaveSchemaPackage(*g_role_provider, kCodecIdCompactIndexed);
  if (!g_role_schema.valid() || !g_role_schema.registerProfile()) {
    Serial.println("[SLAVE][PROFILE] app-owned SENS schema registration failed");
    return;
  }

  SlaveNodeBootstrap::Config boot_cfg{};
  boot_cfg.manager_config = SlaveNodeBootstrap::defaultManagerConfig(1, PCAT_SENS_DEV_TYPE, true, true, 5000);
