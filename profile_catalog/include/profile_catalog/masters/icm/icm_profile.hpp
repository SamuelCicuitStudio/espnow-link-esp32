...
struct IcmDescriptorAppConfig {
  std::string device_type = PCAT_ICM_DEV_TYPE;
  std::string hw_version = PCAT_ICM_HW_VER;
  std::string sw_version = PCAT_ICM_SW_VER;
  std::string build_id = buildid::defaultBuildId(PCAT_ICM_BUILD_ID, PCAT_ICM_BUILD_TAG, PCAT_ICM_SW_VER);
  std::string default_device_name = PCAT_ICM_DEF_NAME;
};

class IcmAppDescriptorProvider : public espnow_link::IDescriptorProvider, public espnow_link::ITelemetryPushProvider {
 public:
  IcmAppDescriptorProvider(espnow_link::PreferencesStore& nvs,
 ...
 private:
  ...
  IcmDescriptorAppConfig cfg_{};
};

class IcmAppProfileDefinition final : public espnow_link::IProfileDefinition {
 public:
...
SlaveSchemaPackage makeIcmSlaveSchemaPackage(IcmAppDescriptorProvider& provider,
                                             espnow_link::CodecId codec_id = espnow_link::kCodecIdCompactIndexed);

MasterSchemaPackage makeIcmMasterSchemaPackage(espnow_link::CodecId codec_id = espnow_link::kCodecIdCompactIndexed);
