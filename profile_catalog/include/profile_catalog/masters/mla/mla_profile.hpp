...
struct MlaDescriptorAppConfig {
  std::string device_type = PCAT_MLA_DEV_TYPE;
  std::string hw_version = PCAT_MLA_HW_VER;
  std::string sw_version = PCAT_MLA_SW_VER;
  std::string build_id = buildid::defaultBuildId(PCAT_MLA_BUILD_ID, PCAT_MLA_BUILD_TAG, PCAT_MLA_SW_VER);
  std::string default_device_name = PCAT_MLA_DEF_NAME;
};

class MlaAppDescriptorProvider : public espnow_link::IDescriptorProvider, public espnow_link::ITelemetryPushProvider {
 public:
  MlaAppDescriptorProvider(espnow_link::PreferencesStore& nvs,
 ...
 private:
  ...
  MlaDescriptorAppConfig cfg_{};
};

class MlaAppProfileDefinition final : public espnow_link::IProfileDefinition {
 public:
...
SlaveSchemaPackage makeMlaSlaveSchemaPackage(MlaAppDescriptorProvider& provider,
                                             espnow_link::CodecId codec_id = espnow_link::kCodecIdCompactIndexed);

MasterSchemaPackage makeMlaMasterSchemaPackage(espnow_link::CodecId codec_id = espnow_link::kCodecIdCompactIndexed);
