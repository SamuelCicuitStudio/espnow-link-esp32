...
#define PCAT_PMS_MET_WALLI_MIN -60.0f       // Minimum expected wall current telemetry (A).
#define PCAT_PMS_MET_WALLI_MAX 60.0f        // Maximum expected wall current telemetry (A).

#define PCAT_PMS_MET_BATTI_MIN -60.0f       // Minimum expected battery current telemetry (A).
#define PCAT_PMS_MET_BATTI_MAX 60.0f        // Maximum expected battery current telemetry (A).
...
#define PCAT_PMS_DESC_MET_WALLI "type=f32;unit=A;min=-60.0;max=60.0;pull=1;push=1"    // Descriptor metadata for wall current.
#define PCAT_PMS_DESC_MET_BATTI "type=f32;unit=A;min=-60.0;max=60.0;pull=1;push=1"    // Descriptor metadata for battery current.
...
#define PCAT_PMS_SETMAP "dname,chan,pmod,LoopAuto,fan_mode,buzzer_enable,led_feedback_enable,rgb_idle_color,rgb_alert_color,rgb_brightness,chain_48v_enable,charger_enable,tripi,vcal,ical,vbovp,vbuvp,ibocp,baovp,bauvp,biocp"  // Ordered setting key map for capabilities.
