...
#include "profile_catalog/slaves/semu/semu_keys.hpp"
...
struct SemuRuntimeTelemetrySnapshot {
  static constexpr uint8_t kMaxPairs = static_cast<uint8_t>(PCAT_SEMU_SET_SCNT_MAX);
...
using SemuReadTelemetryCallback = bool (*)(void* user, SemuRuntimeTelemetrySnapshot& out);
using SemuApplySettingCallback = bool (*)(void* user,
                                          const std::string& key,
                                          const std::string& value,
                                          std::string& out_message);
using SemuSettingFeedbackCallback = void (*)(void* user,
                                             const std::string& key,
                                             const std::string& value,
                                             bool ok);

struct SemuDescriptorAppConfig {
  std::string device_type = PCAT_SEMU_DEV_TYPE;
  std::string hw_version = PCAT_SEMU_HW_VER;
  std::string sw_version = PCAT_SEMU_SW_VER;
  std::string build_id = buildid::defaultBuildId(PCAT_SEMU_BUILD_ID, PCAT_SEMU_BUILD_TAG, PCAT_SEMU_SW_VER);
  std::string default_device_name = PCAT_SEMU_DEF_NAME;
  void* runtime_user = nullptr;
  SemuReadTelemetryCallback read_telemetry = nullptr;
  SemuApplySettingCallback apply_setting = nullptr;
  SemuSettingFeedbackCallback setting_feedback = nullptr;
};

class SemuAppDescriptorProvider : public espnow_link::IDescriptorProvider, public espnow_link::ITelemetryPushProvider {
 public:
  SemuAppDescriptorProvider(espnow_link::PreferencesStore& nvs,
...
SlaveSchemaPackage makeSemuSlaveSchemaPackage(SemuAppDescriptorProvider& provider,
