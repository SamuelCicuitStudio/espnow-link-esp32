...
struct RelayRuntimeTelemetrySnapshot {
  uint32_t relay_bitmap = 0U;
  float env_temp_c = 0.0f;
  bool valid = false;
};

using RelayReadTelemetryCallback = bool (*)(void* user, RelayRuntimeTelemetrySnapshot& out);
using RelayApplySettingCallback = bool (*)(void* user,
                                           const std::string& key,
                                           const std::string& value,
                                           std::string& out_message);
using RelaySettingFeedbackCallback = void (*)(void* user,
                                              const std::string& key,
                                              const std::string& value,
                                              bool ok);

struct RelayDescriptorAppConfig {
  std::string device_type = PCAT_RELAY_DEV_TYPE;
  std::string hw_version = PCAT_RELAY_HW_VER;
  std::string sw_version = PCAT_RELAY_SW_VER;
  std::string build_id = buildid::defaultBuildId(PCAT_RELAY_BUILD_ID, PCAT_RELAY_BUILD_TAG, PCAT_RELAY_SW_VER);
  std::string default_device_name = PCAT_RELAY_DEF_NAME;
  void* runtime_user = nullptr;
  RelayReadTelemetryCallback read_telemetry = nullptr;
  RelayApplySettingCallback apply_setting = nullptr;
  RelaySettingFeedbackCallback setting_feedback = nullptr;
};

class RelayAppDescriptorProvider : public espnow_link::IDescriptorProvider, public espnow_link::ITelemetryPushProvider {
 public:
  RelayAppDescriptorProvider(espnow_link::PreferencesStore& nvs,
...
SlaveSchemaPackage makeRelaySlaveSchemaPackage(RelayAppDescriptorProvider& provider,
