...
struct AlarmRuntimeTelemetrySnapshot {
...
using AlarmReadTelemetryCallback = bool (*)(void* user, AlarmRuntimeTelemetrySnapshot& out);
using AlarmApplySettingCallback = bool (*)(void* user,
                                           const std::string& key,
                                           const std::string& value,
                                           std::string& out_message);
using AlarmSettingFeedbackCallback = void (*)(void* user,
                                              const std::string& key,
                                              const std::string& value,
                                              bool ok);

struct AlarmDescriptorAppConfig {
  std::string device_type = PCAT_ALARM_DEV_TYPE;
  std::string hw_version = PCAT_ALARM_HW_VER;
  std::string sw_version = PCAT_ALARM_SW_VER;
  std::string build_id = buildid::defaultBuildId(PCAT_ALARM_BUILD_ID, PCAT_ALARM_BUILD_TAG, PCAT_ALARM_SW_VER);
  std::string default_device_name = PCAT_ALARM_DEF_NAME;
  void* runtime_user = nullptr;
  AlarmReadTelemetryCallback read_telemetry = nullptr;
  AlarmApplySettingCallback apply_setting = nullptr;
  AlarmSettingFeedbackCallback setting_feedback = nullptr;
};

class AlarmAppDescriptorProvider : public espnow_link::IDescriptorProvider, public espnow_link::ITelemetryPushProvider {
 public:
  AlarmAppDescriptorProvider(espnow_link::PreferencesStore& nvs,
 ...
 private:
  ...
  AlarmDescriptorAppConfig cfg_{};
  mutable std::vector<espnow_link::SettingDescriptor> settings_cache_{};
  mutable bool settings_cache_valid_ = false;
};

class AlarmAppProfileDefinition final : public espnow_link::IProfileDefinition {
 public:
...
SlaveSchemaPackage makeAlarmSlaveSchemaPackage(AlarmAppDescriptorProvider& provider,
