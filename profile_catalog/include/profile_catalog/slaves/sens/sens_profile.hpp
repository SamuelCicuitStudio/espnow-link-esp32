...
using SensReadTelemetryCallback = bool (*)(void* user, SensRuntimeTelemetrySnapshot& out);
using SensApplySettingCallback = bool (*)(void* user,
                                          const std::string& key,
                                          const std::string& value,
                                          std::string& out_message);
using SensSettingFeedbackCallback = void (*)(void* user,
                                             const std::string& key,
                                             const std::string& value,
                                             bool ok);

struct SensDescriptorAppConfig {
  std::string device_type = PCAT_SENS_DEV_TYPE;
  std::string hw_version = PCAT_SENS_HW_VER;
  std::string sw_version = PCAT_SENS_SW_VER;
  std::string build_id = buildid::defaultBuildId(PCAT_SENS_BUILD_ID, PCAT_SENS_BUILD_TAG, PCAT_SENS_SW_VER);
  std::string default_device_name = PCAT_SENS_DEF_NAME;
  void* runtime_user = nullptr;
  SensReadTelemetryCallback read_telemetry = nullptr;
  SensApplySettingCallback apply_setting = nullptr;
  SensSettingFeedbackCallback setting_feedback = nullptr;
};

class SensAppDescriptorProvider : public espnow_link::IDescriptorProvider, public espnow_link::ITelemetryPushProvider {
 public:
  SensAppDescriptorProvider(espnow_link::PreferencesStore& nvs,
 ...
 private:
  ...
  SensDescriptorAppConfig cfg_{};
};

class SensAppProfileDefinition final : public espnow_link::IProfileDefinition {
 public:
...
SlaveSchemaPackage makeSensSlaveSchemaPackage(SensAppDescriptorProvider& provider,
