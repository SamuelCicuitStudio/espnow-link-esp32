...
class ManagementFrontendAdapter {
 ...
 private:
  ...
  void trimEventRing_() {
    if (event_ring_.size() <= event_ring_capacity_) return;
    const size_t drop = event_ring_.size() - event_ring_capacity_;
    event_ring_.erase(event_ring_.begin(),
  ...
  CommandStatusView* findReqStatusMutable_(uint32_t req_id) {
    for (auto& s : req_status_) {
      if (s.req_id == req_id) return &s;
    }
    return nullptr;
  }
