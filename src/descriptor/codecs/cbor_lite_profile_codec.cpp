...
constexpr uint8_t kCborLiteMagicDesc = 0xA1;
constexpr uint8_t kCborLiteMagicCtrl = 0xA2;
constexpr uint8_t kCborLiteVersion = 1;
constexpr uint8_t kCborLiteKindQuery = 1;
constexpr uint8_t kCborLiteKindResponse = 2;
constexpr uint8_t kCborLiteKindCommand = 1;
constexpr uint8_t kCborLiteKindResult = 2;
...
bool wrap(uint8_t magic,
  ...
  out_payload.clear();
  return appendU8(out_payload, magic) && appendU8(out_payload, kCborLiteVersion) && appendU8(out_payload, kind) &&
...
bool unwrap(const uint8_t* payload,
  ...
  if (payload[0] != expected_magic || payload[1] != kCborLiteVersion || payload[2] != expected_kind) return false;
...
bool CborLiteProfileCodec::encodeDescriptorQuery(const DescriptorQuery& query,
                                                 std::vector<uint8_t>& out_payload) const {
  std::vector<uint8_t> inner;
  if (!::espnow_link::encodeDescriptorQuery(query, inner)) {
    return false;
  }
  return wrap(kCborLiteMagicDesc, kCborLiteKindQuery, inner.data(), inner.size(), out_payload);
}

bool CborLiteProfileCodec::decodeDescriptorQuery(const uint8_t* payload,
                                                 size_t len,
                                                 DescriptorQuery& out) const {
  const uint8_t* inner = nullptr;
  size_t inner_len = 0;
  if (!unwrap(payload, len, kCborLiteMagicDesc, kCborLiteKindQuery, inner, inner_len)) {
    return parseDescriptorQuery(payload, len, out);
  }
  return parseDescriptorQuery(inner, inner_len, out);
}

bool CborLiteProfileCodec::encodeDescriptorResponse(const DescriptorResponse& response,
                                                    std::vector<uint8_t>& out_payload) const {
  std::string encoded;
  if (!::espnow_link::encodeDescriptorResponse(response, encoded)) {
    return false;
  }
  return wrap(kCborLiteMagicDesc,
              kCborLiteKindResponse,
              reinterpret_cast<const uint8_t*>(encoded.data()),
              encoded.size(),
              out_payload);
}

bool CborLiteProfileCodec::decodeDescriptorResponse(const uint8_t* payload,
                                                    size_t len,
                                                    DescriptorResponse& out) const {
  const uint8_t* inner = nullptr;
  size_t inner_len = 0;
  if (!unwrap(payload, len, kCborLiteMagicDesc, kCborLiteKindResponse, inner, inner_len)) {
...
bool CborLiteProfileCodec::encodeControlCommand(uint16_t cmd_id,
                                                std::vector<uint8_t>& out_payload) const {
  std::vector<uint8_t> inner;
  if (!buildControlCommandPayload(cmd_id, inner)) {
    return false;
  }
  return wrap(kCborLiteMagicCtrl, kCborLiteKindCommand, inner.data(), inner.size(), out_payload);
}

bool CborLiteProfileCodec::decodeControlCommand(const uint8_t* payload,
                                                size_t len,
                                                uint16_t& out_cmd_id) const {
  const uint8_t* inner = nullptr;
  size_t inner_len = 0;
  if (!unwrap(payload, len, kCborLiteMagicCtrl, kCborLiteKindCommand, inner, inner_len)) {
    return parseControlCommandPayload(payload, len, out_cmd_id);
  }
  return parseControlCommandPayload(inner, inner_len, out_cmd_id);
}

bool CborLiteProfileCodec::encodeControlResult(uint16_t cmd_id,
  ...
  return wrap(kCborLiteMagicCtrl, kCborLiteKindResult, inner.data(), inner.size(), out_payload);
}

bool CborLiteProfileCodec::decodeControlResult(const uint8_t* payload,
                                               size_t len,
                                               uint16_t& out_cmd_id,
                                               uint16_t& out_result_code) const {
  const uint8_t* inner = nullptr;
  size_t inner_len = 0;
  if (!unwrap(payload, len, kCborLiteMagicCtrl, kCborLiteKindResult, inner, inner_len)) {
